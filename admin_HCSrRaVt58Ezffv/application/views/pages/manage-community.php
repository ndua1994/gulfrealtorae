...
<h3 class="card-label">Manage Community</h3>
</div>
...
<!--end::Dropdown-->
<!--begin::Button-->
<a href="<?=base_url('community/add')?>" class="btn btn-primary font-weight-bolder">
<span class="svg-icon svg-icon-md">
...
<table id="example" class="table  table-bordered" style="width:100%">
        ...
        <tbody>
		<?php foreach($community as $key=>$comm){?>
		<tr>
		<td><?=++$key?></td>
		<td>
		<img src="<?=base_url('uploads/'.$comm->comm_img.'')?>" width='120'></td>
		<td><?=$comm->comm_heading?></td>
		<td><?=$comm->comm_short_descp?></td>
		<td><?=$comm->first_name.' '.$comm->last_name?></td>
		<td><?=$comm->updated_at?></td>
		<td><span class="label label-lg font-weight-bold 
		<?=($comm->is_active==1 ? 'label-light-success' : 'label-light-danger')?> label-inline"><?=($comm->is_active==1 ? 'Active' : 'Inactive')?></span></td>
		<td><a href="<?=base_url('community/edit/'.$comm->comm_id.'')?>" class="btn btn-warning">Edit</a></td>
		<td><a href="javascript:void(0)" class="btn btn-danger del_rec" data-url="<?=base_url('community/delete/'.$comm->comm_id.'')?>">Delete</a></td>
		</tr>
		<?php }?>  
        </tbody>
        <tfoot>
