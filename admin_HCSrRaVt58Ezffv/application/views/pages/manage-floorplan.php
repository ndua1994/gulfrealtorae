...
<h3 class="card-label">Manage Floor Plan</h3>
</div>
...
<!--end::Button-->
<!--begin::Button-->
<a href="<?=base_url('property/add-floor-plan/'.$prop_id.'')?>" class="btn btn-primary font-weight-bolder">
<span class="svg-icon svg-icon-md">
<!--begin::Svg Icon | path:assets/media/svg/icons/Design/Flatten.svg-->
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
<g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
...
<table id="example" class="table  table-bordered" style="width:100%">
        ...
        <tbody>
		<?php foreach($floor_plan as $key=>$fp){?>
		<tr>
		<td><?=++$key?></td>
		<td>
		<?php
		$fp_images=explode(',',$fp->floor_imgs);
		foreach($fp_images as $fp_img):
		?>	
		<img src="<?=base_url('uploads/'.$fp_img.'')?>" width='35'>
        <?php endforeach;?>
	    </td>
		<td><?=$fp->floor_name?></td>
		<td><?=$fp->floor_size?></td>
		<td><?=$fp->floor_room?></td>
		<td><?=$fp->floor_bath?></td>
		<td><?=$fp->floor_price?></td>
		<td><span class="label label-lg font-weight-bold 
		<?=($fp->is_active==1 ? 'label-light-success' : 'label-light-danger')?> label-inline"><?=($fp->is_active==1 ? 'Active' : 'Inactive')?></span></td>
		<td><a href="<?=base_url('property/floor-plan-edit/'.$fp->prop_floorplan_id.'')?>" class="btn btn-warning">Edit</a></td>
		<td><a href="javascript:void(0)" class="btn btn-danger del_rec" data-url="<?=base_url('property/floor-plan-delete/'.$fp->prop_floorplan_id.'')?>">Delete</a></td>
		</tr>
		<?php }?>  
        </tbody>
        <tfoot>
