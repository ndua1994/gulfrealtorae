...
<h3 class="card-label">Manage Image</h3>
</div>
...
<!--end::Dropdown-->
<!--begin::Button-->
<a href="<?=base_url('upload-image/add')?>" class="btn btn-primary font-weight-bolder">
<span class="svg-icon svg-icon-md">
...
<table id="example" class="table  table-bordered" style="width:100%">
        ...
        <tbody>
		<?php foreach($upload_image as $key=>$upload){?>
		<tr>
		<td><?=++$key?></td>
		<td>
		<img src="<?=base_url('uploads/'.$upload->upload_img.'')?>" class="img_url" width='120'></td>
		<td><?=$upload->first_name.' '.$upload->last_name?></td>
		<td><?=$upload->updated_at?></td>
		<td><span class="label label-lg font-weight-bold 
		<?=($upload->is_active==1 ? 'label-light-success' : 'label-light-danger')?> label-inline"><?=($upload->is_active==1 ? 'Active' : 'Inactive')?></span></td>
		<td><span class="copy-btn btn btn-info btn-md" data-type="attribute" data-attr-name="data-clipboard-text" data-model="couponCode" data-clipboard-text=<?=base_url('uploads/').$upload->upload_img?>>Copy Image URL</span></td>

		


		<td><a href="<?=base_url('upload-image/edit/'.$upload->upload_id.'')?>" class="btn btn-warning">Edit</a></td>
		<td><a href="javascript:void(0)" class="btn btn-danger del_rec" data-url="<?=base_url('upload-image/delete/'.$upload->upload_id.'')?>">Delete</a></td>
		</tr>
		<?php }?>  
        </tbody>
        <tfoot>
